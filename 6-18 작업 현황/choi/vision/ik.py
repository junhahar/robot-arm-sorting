...
M1_OFFSET, M1_DIR = 180.0, +1
M2_OFFSET, M2_DIR = 180.0, -1   # ★ 방향테스트 반영: 어깨 들기=M2 감소
M4_OFFSET, M4_DIR = 262.0, +1   # 캘리(x195,z31,수직): 실측 매칭. 하드웨어 미세조정
M5_OFFSET, M5_DIR = 224.0, +1   # 캘리: 실측 매칭. 하드웨어 미세조정
M6_OFFSET, M6_DIR = 180.0, +1
...
def homography_to_arm(hx, hy):
    """호모그래피 좌표(mm) → 로봇 팔 좌표(mm). ARM_BASE 평행이동 후 φ 회전."""
    dx = hx - ARM_BASE_X
    dy = hy - ARM_BASE_Y
    phi = np.radians(HOMO_ROT_DEG)
...
def solve_ik(x, y, bolt_angle_deg, z_grasp=GRASP_HEIGHT):
    ...
    cmd = {
        ...
        3: M2_OFFSET - M2_DIR * theta2,    # M3: M2 mirror
