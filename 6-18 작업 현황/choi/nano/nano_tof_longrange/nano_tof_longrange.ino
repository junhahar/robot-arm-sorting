...
//   ★ 변형: LONG RANGE (사거리) 모드 시도  ─ 실패 시 자동 폴백
//   ★ ToF 라이브러리: Adafruit_VL53L0X
//
//   [VL53L0X(TOF200C)] ──I2C(A4/A5)── [Nano] ──SPI(D10~D13)── [MCP2515] ──CAN── [RPi5]
//
// [⚠ 매우 중요 - 읽어볼 것]
//   진짜 LONG_RANGE 프로파일은 레이저 펄스폭(VCSEL period)을 바꾸고,
//   그 과정에서 센서 내부 재캘리브레이션이 일어난다.
//   TOF200C 같은 "클론" 모듈은 이 재캘리브레이션을 거부해서
//   configSensor(LONG_RANGE)가 false를 반환하는 경우가 많다(우리 모듈에서 확인됨).
//
//   그래서 이 스케치는:
//     1) 먼저 기본 begin()으로 깨끗하게 초기화하고
//     2) LONG_RANGE 적용을 "시도"한 뒤
//     3) 거부당하면 기본 모드로 자동 폴백한다.
//   → 모듈이 받아주면 ~2m, 거부하면 기본(약 1m, 타깃/조명에 좌우)으로 동작.
//
//   ※ 클론에서 사거리를 늘리는 더 확실한 방법은 적분시간을 키우는 것이며,
//     그건 nano_tof_highaccuracy.ino (HIGH_ACCURACY, 200ms) 쪽이다.
//
// [CAN 프레임]  ID=0x300, DLC=8, little-endian  (다른 버전과 동일)
...
const uint16_t SEND_PERIOD_MS = 100;     // LONG_RANGE는 적분 33ms → 10Hz 유지
...
void setup() {
  Serial.begin(115200);
  delay(500);
  Serial.println(F("=== Nano ToF CAN sender (LONG RANGE 시도) ==="));

  // --- VL53L0X 초기화: 기본 begin() → LONG_RANGE 시도 → 실패 시 폴백 ---
  Wire.begin();
  if (tof.begin()) {                 // 1) 기본 초기화 (클린)
    if (tof.configSensor(Adafruit_VL53L0X::VL53L0X_SENSE_LONG_RANGE)) {
      tofReady = true;               // 2) 모듈이 받아줌 → ~2m
      Serial.println(F("VL53L0X init OK (LONG_RANGE ~2m 적용됨)"));
    } else {
      // 3) 클론이 거부 → 센서가 어중간해질 수 있으니 기본으로 재초기화
      Serial.println(F("LONG_RANGE 거부됨(TOF200C 클론) -> 기본 모드로 폴백"));
      tofReady = tof.begin();
      if (tofReady) Serial.println(F("VL53L0X 기본 모드 재초기화 OK"));
      else          Serial.println(F("재초기화 실패 <- USB 완전히 뽑았다 꽂기"));
    }
  } else {
    tofReady = false;
  ...
  Serial.println(F("Sender ready. ToF ID=0x300, period=100ms (10Hz)"));
}
