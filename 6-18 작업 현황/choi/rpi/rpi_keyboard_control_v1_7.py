#!/usr/bin/env python3
"""
rpi_keyboard_control_v1_7.py — v1_6 + MG90 그리퍼(Nano) 키 제어
================================================================
v1_6 대비 바뀐 점:
  · MG90 그리퍼를 Nano로 제어. RPi가 CAN ID 0x310으로 각도(0~180°)를 송신하면
    Nano(nano_tof_mg90.ino)가 받아 PWM(D9)으로 MG90을 구동한다.
  · STS3215 6축(0x100)은 기존과 동일. 그리퍼만 별도 노드/ID로 분리.
...
실행:     python3 rpi_keyboard_control_v1_7.py
"""
...
CAN_ID_CMD = 0x100
CAN_ID_TELEM = 0x201
CAN_ID_GRIPPER = 0x310        # ★ RPi → Nano MG90 그리퍼 명령 (byte[0]=각도 0~180)
CMD_TORQUE = 0x02
CMD_SET_ANGLE = 0x03
CMD_SET_MIDDLE = 0x04
...
# ── 카메라 촬영 자세 (모터별 명령각) ──
CAMERA_POSE = {1: 180.0, 2: 107.4, 3: 245.6, 4: 107.4, 5: 129.2, 6: 180.0}    # 9번
CAMERA_POSE2 = {1: 180.0, 2: 100.7, 3: 252.3, 4: 144.2, 5: 170.8, 6: 180.0}   # 8번

# ── MG90 그리퍼 파라미터 (실제 기구에 맞게 튜닝) ──
# 다른 서보축처럼 'hold-to-move' — y/h 누르는 동안 속도가 램프되며 연속 이동.
GRIPPER_INIT = 90              # 부팅 시 각도[deg]
GRIPPER_STEP = 2               # y/h 한 번 누름 = ±2° (톡톡 이산). 꾹 누르면 자동반복으로 또박또박
# ⚠ 실제 열림/닫힘 각도를 측정해 범위를 좁히세요 (기구 과구동 방지). 예: MIN=40, MAX=95
GRIPPER_MIN = 0
GRIPPER_MAX = 180
...
def send_gripper(bus, angle):
    """MG90 그리퍼 각도(0~180°)를 Nano(0x310)로 송신. 클램프 후 보낸 값 반환."""
    angle = max(GRIPPER_MIN, min(GRIPPER_MAX, int(round(angle))))
    data = [angle & 0xFF, 0, 0, 0, 0, 0, 0, 0]
    try:
        bus.send(can.Message(arbitration_id=CAN_ID_GRIPPER, data=data, is_extended_id=False))
    except can.CanError:
        pass
    return angle
...
def main():
    ...
    last_seen = {}
    homing = set()
    home_start = {}    # mid → (start_angle, start_time, duration)
    pose_idx = None
    pose_goal = CAMERA_POSE
    pose_start = {}    # mid → (start_angle, start_time, duration, goal)
    gripper_angle = send_gripper(bus, GRIPPER_INIT)    # 초기 송신, 반환=정수 각도

    print("키보드 제어 v1_7 시작 (STS3215 + MG90 그리퍼)")
    ...
    print("  그리퍼(MG90): y=+2°  h=-2° (톡톡 누름, 꾹=또박또박)        ESC=종료")
    print("  상태줄: M#화살표 목표/실측  (H=홈복귀, P=자세이동), grip=그리퍼각\n")
        ...
        while True:
            ...
            while select.select([sys.stdin], [], [], 0)[0]:
                ...
                if ch in MOVE_KEYS:
                    ...
                    pose_idx = None
                    pose_start.clear()
                elif ch in "123456":  # 해당 모터 홈복귀 (S-curve)
                    hm = int(ch)
                    if 1 <= hm <= MOTOR_COUNT:
                        cur = target[hm] if target[hm] is not None else measured.get(hm)
                        if cur is None:
            ...
            if pose_idx is not None:
                ...
                if all(mid not in pose_start for mid, _s in members):
                    ...
                    for mid, _s in members:
                        cur = target[mid] if target[mid] is not None else measured.get(mid)
                        if cur is None:
                            ready = False
                            break
                        seeds[mid] = cur
                    if ready:
                        maxdist = max(abs(pose_goal[mid] - seeds[mid]) for mid, _s in members)
            ...
            if now >= next_status:
                ...
                sys.stdout.write("\r" + "  ".join(parts)
                                 + f" |pre{pre:+.1f} grip{gripper_angle}  ")
                sys.stdout.flush()
