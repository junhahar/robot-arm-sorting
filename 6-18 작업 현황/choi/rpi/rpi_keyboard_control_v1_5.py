#!/usr/bin/env python3
"""
rpi_keyboard_control_v1_5.py — v1_4 + 홈복귀를 S-curve로 (9번 자세는 기존 유지)
================================================================
v1_4 대비 바뀐 점:
  · M4 gain 5 → 2
  · STREAM_MOVE_MS 40 → 80 (명령 간 보간 부드럽게, 홈복귀 끊김 완화)
  · 홈복귀(0, 1~6)를 'S-curve(smoothstep) 시간기반 궤적'으로 → 가감속 부드럽게, 끊김 제거
  · 9번 카메라 자세는 '비례감속 방식' 유지 (전용 변수 CAMERA_MAX_SPEED 등으로 분리)

키 매핑
  이동:  M1 q/a   M2·M3 w/s(거울)   M4 e/d   M5 r/f   M6 t/g
  영점:  z=M1  x=M2·M3  c=M4  v=M5  b=M6   (180° 중위보정)
  홈복귀(S-curve): 1~6 그 모터 180°,  0 전체 180°
  카메라 자세: 9  (M1→M2·3→M5→M4→M6 순차)
  프리로드(M2·3): ] = +0.5,  [ = -0.5      종료: ESC

CAN 셋업: sudo ip link set can0 type can bitrate 125000 sample-point 0.625
실행:     python3 rpi_keyboard_control_v1_5.py
"""
...
RATE_HZ = 40.0
DT = 1.0 / RATE_HZ
STREAM_MOVE_MS = 80    # 40→80: 명령 간 보간 부드럽게 (끊김 완화). 수동 이동에도 적용됨
RAMP_RATE = 20.0
MAX_SPEED = 20.0
HOLD_TIMEOUT = 0.5

# ── 홈복귀(S-curve) 파라미터 ──
HOME_SPEED = 20.0         # 홈복귀 평균 속도[deg/s] (이동시간 = 거리/이 값)
HOME_MIN_DURATION = 0.8   # 짧은 이동도 최소 이 시간[s] 동안 (너무 짧으면 툭 함 방지)

# ── 카메라 자세(9번) 이동 파라미터 — 비례감속 방식 ──
CAMERA_MAX_SPEED = 20.0
CAMERA_APPROACH_GAIN = 0.08
CAMERA_DEADBAND = 0.5
...
def approach(target, mid, goal, send_fn, step_cap):
    """비례감속: target[mid]을 goal로 한 틱 이동. 도착하면 True. (9번 자세용)"""
    remaining = goal - target[mid]
    if abs(remaining) <= CAMERA_DEADBAND:
        target[mid] = goal
        send_fn(mid, goal)
        return True
    step = remaining * CAMERA_APPROACH_GAIN
    if step > step_cap:
        step = step_cap
    elif step < -step_cap:
        step = -step_cap
...
def main():
    ...
    last_seen = {}
    homing = set()
    home_start = {}   # mid → (start_angle, start_time, duration)  S-curve 홈복귀용
    pose_idx = None

    print("키보드 제어 v1_5 시작 (홈복귀 S-curve + 카메라 자세 9번)")
    ...
    try:
        ...
        next_status = 0.0
        next_hold = 0.0
        camera_step_cap = CAMERA_MAX_SPEED * DT   # 9번 자세용
        ...
        while True:
            ...
            # 2.3) S-curve 홈복귀 (smoothstep 시간기반)
            ...
            # 2.4) 카메라 자세 (기존 비례감속) — 현재 축만, 도착하면 다음 축
            posing_motors = set()
                ...
                for mid, _sign in ax["members"]:
                    ...
                    if target[mid] is None:
                        ...
                        if a is None:
                            ...
                            continue
                        target[mid] = a
                    if not approach(target, mid, CAMERA_POSE[mid], send_fn, camera_step_cap):
                        axis_done = False
                if axis_done:
                    pose_idx += 1
            ...
            # 2.5) 프리로드 축 정지 중 재송신 (홈복귀/자세이동 중 제외)
            if now >= next_hold:
                next_hold = now + 0.3
                for ai, ax in enumerate(AXES):
