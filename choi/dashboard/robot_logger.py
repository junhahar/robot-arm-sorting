...
CLI
  python3 robot_logger.py --init      # DB 파일 + 표 6개 생성(있으면 그대로 둠)
...
TABLES = ("run_session", "motor_sample", "step_event", "sort_event", "grip_event", "system_event")
...
class RobotLogger:
    ...
    def __init__(self, db_path=DEFAULT_DB, batch_size=200, flush_interval=0.5,
        ...
        self._lock = threading.RLock()   # 재진입: 브리지 CAN스레드(log_motor)+제어스레드(set_step/log_*)가 같은 conn 동시접근 → 직렬화
    ...
    def log_sort(self, object_type, bin=None, success=None, yolo_conf=None,
                 ...
                 part_id=None, dest=None):
        with self._lock:
            self.conn.execute(
                "INSERT INTO sort_event"
                "(ts, session, cycle_no, part_id, object_type, dest, bin, success, yolo_conf, tof, corr_dx, corr_dy, cycle_sec) "
                "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (time.time(), self.session_id, self.cycle_no, part_id, object_type, dest, bin,
                 None if success is None else int(bool(success)),
                 yolo_conf, tof, corr_dx, corr_dy, cycle_sec),
            )
            self.conn.commit()

    def log_grip(self, part_id=None, phase=None, tof_mm=None,
                 threshold_mm=None, result=None):
        """파이프 ToF 파지 판정 이벤트(인피드 집기 / CNC 회수 집기). 파이프만 호출.
        phase: 'infeed_grip' | 'retrieve_grip'. result: 1 잡힘 / 0 놓침. part_id 로 sort_event 와 조인."""
        with self._lock:
            self.conn.execute(
                "INSERT INTO grip_event"
                "(ts, session, cycle_no, part_id, phase, tof_mm, threshold_mm, result) "
                "VALUES(?,?,?,?,?,?,?,?)",
                (time.time(), self.session_id, self.cycle_no, part_id, phase, tof_mm, threshold_mm,
                 None if result is None else int(bool(result))),
            )
            self.conn.commit()
...
def cmd_info(db_path):
    ...
    if missing:
        ...
        return 1
    print("OK: 표 %d개 모두 존재." % len(TABLES))
    return 0
    ...
    try:
        ...
        for cyc in range(1, 4):
            ...
            log.log_sort("bolt" if cyc % 2 else "nut", bin="A", success=True,
                         ...
                         cycle_sec=7.5)
            log.log_grip(part_id=cyc, phase="infeed_grip", tof_mm=84.0,
                         threshold_mm=130.0, result=True)
        ...
        ok = (c["run_session"] == 1 and c["motor_sample"] == expected_motor
              ...
              and c["grip_event"] == 3 and c["system_event"] >= 2)
