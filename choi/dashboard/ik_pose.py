...
H_ARM_POSE = np.array([
    [+0.04201335, +0.96008568, +66.210478],
    [+0.95069721, +0.04593117, -27.420438],
    [+0.00001241, -0.00119697, +1.000000],
])
...
_RBF_LEFT = [
    ((193.3, 119.0), (5.6, 3.4)),       # 좌상 (3회 평균 잔차)
    ((193.0,   7.1), (1.5, 1.5)),       # 좌하
]
_RBF_LEFT_SIGMA = 40.0

_RBF_RIGHT = [
    ((18.0,  82.0), (23.64,  -0.42)),   # 우(far): 아래30 보정
    ((44.0,  71.0), (10.52,   1.12)),   # 우: 아래25
    ((38.0,  32.0), (6.53,   -3.53)),   # 우: 아래15
    ((-8.0,   6.0), (29.12,  30.38)),   # 우끝: 오른30+아래30
]
...
_ADD_PUSH, _ADD_FADE = 20.0, 10.0        # 반경 밀어내기 / 경계 페이드 폭 [mm] (외삽 압축오차가 10보다 커 20으로)
...
_LR_PUSH = 5.0          # 좌우 끝 반경 보정량(mm). 더/덜 가면 이 값만 조정
