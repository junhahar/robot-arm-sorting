#!/usr/bin/env python3
"""
rpi_keyboard_control_v1_2.py — v1(전부 180° 안전) + '부드러운 홈복귀'
================================================================
v1과 동일하되, 홈복귀(0, 1~6)를 한 방에 쏘지 않고 매 틱 조금씩 보내
  · 멀 땐 일정 속도(HOME_MAX_SPEED)로 다가오고
  · 가까울수록 속도가 줄어 살살 멈춤 (남은거리 × gain → 감속)
→ "팍 돌아옴" 없이 안전하게 원점 복귀.

키 매핑
  이동(앞/뒤):  M1 q/a   M2·M3 w/s(거울)   M4 e/d   M5 r/f   M6 t/g
  영점(전부 180° 중위보정, 안 움직임):  z=M1  x=M2·M3  c=M4  v=M5  b=M6
  홈복귀(부드럽게): 1~6 그 모터를 180°로,  0 전체 180°로  (이동 키 누르면 취소)
  프리로드(M2·3 분담): ] = +0.5,  [ = -0.5
  종료: ESC 또는 Ctrl-C

CAN 셋업: sudo ip link set can0 type can bitrate 125000 sample-point 0.625
실행:     python3 rpi_keyboard_control_v1_2.py
"""
...
CMD_TORQUE = 0x02
CMD_SET_ANGLE = 0x03
CMD_SET_MIDDLE = 0x04  # 180° 중위보정만 사용 (안전)
...
# ── 부드러운 홈복귀 파라미터 (여기서 튜닝) ──
HOME_MAX_SPEED = 20.0      # 홈복귀 최대 속도[deg/s] (멀 때). 일반 이동과 동일. 낮추면 더 느림
HOME_APPROACH_GAIN = 0.08  # 감속 세기: 매 틱 (남은거리 × 이 값)만큼 이동 → 가까울수록 느려짐
HOME_DEADBAND = 0.5        # 목표 ±이 각도 안이면 도착으로 보고 멈춤 [deg]
...
def main():
    ...
    last_seen = {}
    homing = set()   # 부드러운 홈복귀 중인 모터들

    print("키보드 제어 v1_2 시작 (전부 180° + 부드러운 홈복귀)")
    print("  이동:  M1 q/a   M2·3 w/s(거울)   M4 e/d   M5 r/f   M6 t/g")
    print("  영점:  z=M1  x=M2·3  c=M4  v=M5  b=M6   (전부 180° 중위보정, 안 움직임)")
    print("  홈복귀(부드럽게): 1~6 그 모터 180°,  0 전체 180°  (이동키 누르면 취소)")
    print("  프리로드(M2·3): ] +0.5 / [ -0.5")
    print("  ESC=종료\n")
    ...
    try:
        ...
        next_status = 0.0
        next_hold = 0.0
        home_step_cap = HOME_MAX_SPEED * DT   # 한 틱 최대 이동량[deg]
        ...
        while True:
            ...
            while select.select([sys.stdin], [], [], 0)[0]:
                ...
                if ch in MOVE_KEYS:
                    ...
                    sys.stdout.write(f"\n[프리로드] M2·3 = {p:+.1f}°\n")
                    sys.stdout.flush()
                elif ch in CALIB_KEYS:  # 영점: 180 중위보정 (안전, 안 움직임)
                    ai = CALIB_KEYS[ch]
                    msgs = []
                    ...
                    sys.stdout.write("\n[영점] " + "  ".join(msgs) + "\n")
                    sys.stdout.flush()
                elif ch == "0":  # 전체 홈복귀(부드럽게)
                    for hm in range(1, MOTOR_COUNT + 1):
                        homing.add(hm)
                    for ai in axis_speed:
                        axis_speed[ai] = 0.0
                elif ch in "123456":  # 해당 모터 홈복귀(부드럽게)
                    hm = int(ch)
                    if 1 <= hm <= MOTOR_COUNT:
                        homing.add(hm)
            ...
            # 2) 축별 수동 이동 (홈복귀 중인 모터는 키 없으면 안 건드림)
            ...
            # 2.3) 부드러운 홈복귀 — 남은거리에 비례해 이동(가까울수록 감속)
            for mid in list(homing):
                if target[mid] is None:
                    a = measured.get(mid)
                    if a is None:
                        continue       # 현재각 받기 전엔 대기(점프 방지)
                    target[mid] = a
                ...
                if abs(remaining) <= HOME_DEADBAND:
                    ...
                    homing.discard(mid)
                    continue
                step = remaining * HOME_APPROACH_GAIN          # 가까울수록 작아짐 → 감속
                if step > home_step_cap:                        # 멀 땐 속도 상한
                    step = home_step_cap
            ...
            # 2.5) 프리로드 축 정지 중 재송신 (홈복귀 중인 모터는 제외)
            if now >= next_hold:
                next_hold = now + 0.3
                for ai, ax in enumerate(AXES):
