#!/usr/bin/env python3
"""
rpi_keyboard_control_v1_11.py — v1_10 + 1~5 축별 목표각 입력이동(S-curve)
================================================================
v1_10 대비 바뀐 점:
  · ★ '1~5' 키: 그 축의 목표각 1개를 입력 → 그 축만 S-curve로 이동.
      1=M1   2=M2·3(거울쌍, M3은 180대칭 반대각)   3=M4   4=M5   5=M6
    즉 7번(6개 한 번에)을 축별로 하나씩 하는 버전. 7번은 그대로 유지.
    한계검사: 입력각(거울쌍은 양쪽 다)이 MOTOR_LIMITS 밖이면 이동 거부+경고.
  · ★ 기존 '1~6' 개별 홈복귀는 제거. 전체 홈복귀 '0'(전부 180°)만 유지. '6'은 미사용.

v1_9 대비 추가된 것 (캘리브 k/j/p/l, 이동·영점·홈·자세·그리퍼 전부 그대로):
  · ★ '7' 키: 각 5개(M1 M2·3 M4 M5 M6)를 입력 → 그 자세로 S-curve 이동.
    M2·3은 한 값으로 받아 M3=360-M2 거울 자동(카메라 IK각 6개 중 M3은 빼고 입력).
...
기존 키 매핑
  ...
  ★축이동(S-curve): 1=M1  2=M2·3  3=M4  4=M5  5=M6 (목표각 1개 입력→이동)
  전체 홈복귀(S-curve): 0 (전부 180°)
...
CAMERA_POSE = {1: 180.0, 2: 104.8, 3: 255.2, 4: 112.7, 5: 146.0, 6: 180.0}  # 9번
CAMERA_POSE2 = {1: 180.0, 2: 100.7, 3: 259.3, 4: 144.2, 5: 170.8, 6: 180.0}  # 8번
...
# ── 축별 목표각 입력이동 키 (v1_11) ──
# AXES 순서 [M1, M2·3, M4, M5, M6] 가 키 1~5에 1:1 대응.
#   1=M1  2=M2·3(거울)  3=M4  4=M5  5=M6.  (기존 개별 홈복귀 1~6은 제거)
AXIS_MOVE_KEYS = {str(i + 1): AXES[i] for i in range(len(AXES))}  # '1'~'5' → 축 dict
...
def prompt_pose_angles(fd, old_attr):
    """raw 모드를 잠깐 풀고 자세이동용 각 5개(M1 M2·3 M4 M5 M6)를 입력받아
    dict{mid:deg}(M1..M6) 반환(취소면 None). M2·3은 한 값으로 받아 거울 자동:
    M2=입력, M3=360-입력. 0~360 클램프만; 한계검사는 호출부(7번)."""
    termios.tcsetattr(fd, termios.TCSADRAIN, old_attr)  # 일반 입력 모드로 복귀
    try:
        sys.stdout.write(
            "\n[자세입력] 각 5개 입력 (M1 M2·3 M4 M5 M6, 공백구분). M3은 거울 자동.\n"
            "           취소=빈줄\n  POSE> "
        )
        sys.stdout.flush()
        line = sys.stdin.readline().strip()
        ...
        vals = line.replace(",", " ").split()
        if len(vals) != 5:
            print(f"[자세입력] 5개 필요 (M1 M2·3 M4 M5 M6) — 입력 {len(vals)}개, 취소")
            return None
        v = [max(ANGLE_MIN, min(ANGLE_MAX, float(x))) for x in vals]  # 0~360 클램프
        m1, m23, m4, m5, m6 = v
        pose = {
            1: m1,
            2: m23,
            3: 2 * CALIB_NEUTRAL - m23,  # M3 = 360 - M2 (180 거울 대칭)
            4: m4,
            5: m5,
            6: m6,
        }
        print(
            "[자세입력] 목표: "
            + "  ".join(f"M{m}={pose[m]:.1f}" for m in range(1, 7))
            + "  (M3 거울 자동)"
        )
...
def prompt_axis_angle(fd, old_attr, label, mids):
    """raw 모드를 잠깐 풀고 한 축의 목표각 1개를 입력받아 float 반환(취소면 None).
    label=화면표시용 축이름, mids=그 축의 모터들(거울쌍이면 여러 개). 한계검사는 호출부."""
    termios.tcsetattr(fd, termios.TCSADRAIN, old_attr)
    try:
        mlist = "·".join(f"M{m}" for m in mids)
        sys.stdout.write(
            f"\n[축이동] {label}({mlist}) 목표각 1개 입력. 취소=빈줄\n  {label}> "
        )
        sys.stdout.flush()
        line = sys.stdin.readline().strip()
        if not line:
            print("[축이동] 취소")
            return None
        deg = float(line.replace(",", " ").split()[0])
        deg = max(ANGLE_MIN, min(ANGLE_MAX, deg))  # 0~360 클램프(한계검사 아님)
        print(f"[축이동] {label} 목표 {deg:.1f}° → S-curve 이동")
        return deg
    except (ValueError, IndexError):
        print("[축이동] 숫자 변환 실패 — 취소")
        return None
    finally:
        tty.setcbreak(fd)
...
def main():
    ...
    print("키보드 제어 v1_11 시작 (STS3215 + MG90, 캘리브 + 7번 자세 + 1~5 축이동)")
    print("  이동:  M1 q/a   M2·3 w/s(거울)   M4 e/d   M5 r/f   M6 t/g")
    print("  영점:  z=M1  x=M2·3  c=M4  v=M5  b=M6   (180° 중위보정)")
    print("  ★축이동(S-curve): 1=M1 2=M2·3 3=M4 4=M5 5=M6 (목표각 입력→이동)")
    print("  전체 홈복귀(S-curve): 0 (전부 180°)")
        ...
        while True:
            ...
            while select.select([sys.stdin], [], [], 0)[0]:
                ...
                if ch in MOVE_KEYS:
                    ...
                    sys.stdout.write("\n[영점] " + "  ".join(msgs) + "\n")
                    sys.stdout.flush()
                elif ch == "0":  # 전체 홈복귀 (전부 180°, S-curve)
                    t0 = time.monotonic()
                    for hm in range(1, MOTOR_COUNT + 1):
                        ...
                        home_start[hm] = (cur, t0, dur, HOME_ANGLE)  # goal=180
                        homing.add(hm)
                    for ai in axis_speed:
                        axis_speed[ai] = 0.0
                    pose_idx = None
                    pose_start.clear()
                elif ch in AXIS_MOVE_KEYS:  # ★ 1~5 = 그 축 목표각 입력 → S-curve 이동
                    ax_def = AXIS_MOVE_KEYS[ch]
                    mids = [m for m, _s in ax_def["members"]]
                    for ai in axis_speed:
                        axis_speed[ai] = 0.0
                    last_seen.clear()
                    deg = prompt_axis_angle(fd, old_attr, ax_def["label"], mids)
                    next_status = 0.0
                    if deg is not None:
                        # 거울쌍이면 멤버별로 sign 적용한 목표각 계산 후 한계검사
                        goals = {}
                        base = AXES[MIRROR_AXIS_IDX]["members"][0][
                            1
                        ]  # 기준 sign(보통 +1)
                        for m, sign in ax_def["members"]:
                            # 단일축은 sign=+1. 거울쌍은 M2=deg, M3=360-deg(180 대칭) 형태로.
                            g = (
                                deg
                                if sign == base or len(ax_def["members"]) == 1
                                else (2 * CALIB_NEUTRAL - deg)
                            )
                            goals[m] = g
                        bad = check_pose_limits(goals)
                        if bad:
                            sys.stdout.write(
                                "\n[축이동] ❌ 한계 밖이라 이동 안 함: "
                                + "  ".join(
                            ...
                            for m, g in goals.items():
                                ...
                                if cur is None:
                                    cur = DEFAULT_ANGLE
                                dur = max(abs(g - cur) / HOME_SPEED, HOME_MIN_DURATION)
                                home_start[m] = (cur, t0, dur, g)
                                homing.add(m)
                            pose_idx = None
                            pose_start.clear()
                    continue
            ...
            # 2.3) S-curve 홈복귀/축이동 (home_start의 goal까지 S-curve)
            #   home_start[mid] = (시작각, 시작시각, 소요, 목표각). 0번=180, 1~5=입력값.
            for mid in list(homing):
                if mid not in home_start:
                    homing.discard(mid)
                    continue
                sa, st, dur, goal = home_start[mid]
